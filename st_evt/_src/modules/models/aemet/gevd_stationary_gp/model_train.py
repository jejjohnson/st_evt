...
from st_evt._src.modules.models.aemet.gevd_stationary_gp.model_zoo import init_t2m_stationary_gp_model, init_feature_transformer
...
def train_model_laplace(
    ...
    logger.debug(f"Saved Transformation: {save_path.joinpath('feature_transforms.joblib')}")
    ...
    model = init_t2m_stationary_gp_model(
    ...
    end_lr = 1e-4
    method = "laplace"
    svi_learner = SVILearner(model, peak_lr=peak_lr, end_lr=end_lr, init_lr=init_lr, num_steps=num_iterations, num_warmup_steps=num_warmup_steps, method=method)

    logger.info("Training Model...")
    svi_posterior = svi_learner(t=t_train, s=s_train, y=y_train, train=True, noise=include_train_noise)
    ...
    posterior_predictive_samples = svi_posterior.posterior_predictive_samples(
        ...
        s=s_pred, t=t_train, train=False, noise=include_pred_noise
    )
    # PREDICTIVE POSTERIOR
    ...
    az_ds_postpred.posterior_predictive["nll"] =  (("chain", "draw", covariate, SPATIAL_DIM_NAME), np.expand_dims(nll_postpred_samples[variable],0))
    ...
    az_ds_postpred.posterior_predictive[f"{variable}_true"] =  ((covariate, SPATIAL_DIM_NAME), np.asarray(y_pred))
    ...
    save_path.mkdir(parents=True, exist_ok=True)
    save_name = (
        "az_stationary_gp_lap_redfeten.zarr"
        if red_feten
        else "az_stationary_gp_lap.zarr"
    )
...
def train_model_mcmc(
    ...
    end_lr = 1e-4
    method = "map"
    svi_learner = SVILearner(model, peak_lr=peak_lr, end_lr=end_lr, init_lr=init_lr, num_steps=num_map_warmup, num_warmup_steps=num_warmup_steps, method=method)
    ...
    mcmc_posterior = mcmc_learner(t=t_train, s=s_train, y=y_train, train=True, noise=include_train_noise)
    ...
    save_path.mkdir(parents=True, exist_ok=True)
    save_name = (
        "az_stationary_gp_mcmc_redfeten.zarr"
        if red_feten
        else "az_stationary_gp_mcmc.zarr"
    )
    logger.debug(f"Filename: {save_name}")
    az_ds.to_zarr(store=str(save_path.joinpath(save_name)))
    logger.debug(f"Full Save Path: {save_path.joinpath(save_name)}")
